...
Railway project **nellits** holds every Nellits app. Accounts runs as service **accounts** with its own
Postgres (**accounts-db**) and a volume mounted at `/data`. Health checks pass (`{"ok":true}`) and
`/.well-known/jwks.json` serves the `nellits-1` key.
...
Railway cannot move a service between projects, so new apps should be created **inside** `nellits`
rather than in a project of their own.
...
Until DNS propagates, `https://accounts.nellits.com` will not resolve. Cookie SSO (`Domain=.nellits.com`)
only works over HTTPS on that host. The service is reachable meanwhile at its Railway domain.
...
RSA keys live in Railway env as `AUTH_RSA_PRIVATE_KEY` / `AUTH_RSA_PUBLIC_KEY` (`AUTH_KEY_ID=nellits-1`).
